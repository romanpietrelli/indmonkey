...
import { getCategorias, crearProductoConVariantes, crearCategoria, actualizarCategoria, eliminarCategoria } from "@/lib/actions";
    ...
    startTransition(async () => {
      ...
      if (imageFiles.length > 0) {
        ...
        try {
          ...
          finalImagenesUrls = await Promise.all(uploadPromises);
          if (finalImagenesUrls.length > 0) {
            finalImageUrl = finalImagenesUrls[0];
          }

        } catch (err: any) {
          toast("error", "Error en subida: " + err.message);
          setIsUploading(false);
          return;
        }
        setIsUploading(false);
      ...
      const res = await crearProductoConVariantes(payload);
      if (res.ok) {
        toast("success", "Producto e imagen guardados en la nube");
        setDone(true);
        setTimeout(() => {
          onSuccess();
          onClose();
                ...
                <AnimatePresence mode="wait">
                  ...
                  {step === 1 && !done && (
                    ...
                    >
                      ...
                      <Field label={`Fotos del Producto (${imageFiles.length})`}>
                        <div
